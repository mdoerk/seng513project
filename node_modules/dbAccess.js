 ...
 *
 * IMPORTANT:
 *			when specificying paramesters in values or conditions, use double quotation ".." on values
...
exports.create = function(table, params, call_back)
{
	...
	if(params.values.length>0)
	{
		...
		for(var index in params.values)
		{
			...
			// Trapping for index insertion
			if (splitResults[0]=="title"){
				indexTitle = splitResults[1];
			}
			if (splitResults[0]=="description"){
				indexDescription = splitResults[1];
			}
		...
		db.open(DATABASE_NAME, function(open_error) {
			// Cant open database - then return and call_back errors
			...
			db.execute(query, function (execute_error){
				...
				db.execute("SELECT last_insert_rowid() \"id\";", function (execute_error, results) {
					if (execute_error) {
						util.log("Error in dbAccess.create(). Can't get id of row we just inserted..");
						if (call_back instanceof Function) { call_back(execute_error, -1); }
					}
					else {
						if (table=="issues"){
							// ** TRAP ** if issues, perform insert
							issueId = results[0].id;
							console.log("- BEGIN INDEX issueID: "+ issueId);
							var parseWords = indexMaker.parse(indexTitle+" "+indexDescription);
							console.log("- - parsing ["+parseWords+"]@issue:"+issueId);
							indexMaker.insertIndex(parseWords, issueId);
						}
