...
ViewProfile.display = function(request, response)
{
	...
	dbAccess.find('users', { conditions:['id="' + id + '"'] }, function(error, results) 
	{
		...
		// Content
		if(results.length != 1) // Found no or many profiles
		{
		...
		else
		{
			...
			response.write('<br>');
			response.write('<strong>Facebook Account</strong> ');
			if(results[0].facebook_account != undefined) {response.write(results[0].facebook_account);}
			else {response.write("None");}
			response.write('<br>');
			response.write('<strong>Twitter Account</strong> ');
			if(results[0].twitter_account != undefined) {response.write(results[0].twitter_account);}
			else {response.write("None");}
			response.write('<br>');
			response.write('</h2>');
		}
