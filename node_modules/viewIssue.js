			...
			dbAccess.runQuery(sqlQuery, function (error, rows) {
				...
				dbAccess.runQuery(sqlQuery2, function (error, rows2) {
					...
					variables = {
						...
						numFollowers: getStats(thisIssueId, 1),
						numComments: getStats(thisIssueId, 2)
					}
...
function getStats(thisIssueId, whichStats) {

var returnstat;

	if (whichStats == 1) // find number of followers
		var sqlQuery= "SELECT count (*) FROM follows WHERE issue_id = " + thisIssueId;
	else if (whichStats == 2) // find number of comments
		var sqlQuery= "SELECT count (*) FROM comments  WHERE issue_id = " + thisIssueId;
	else if (whichStats == 3) // find view count
		var sqlQuery= "SELECT views FROM issues WHERE id = " + thisIssueId;
	
	dbAccess.runQuery(sqlQuery, function (error, results) {
	
		if (error) throw error;
		console.log(whichStats + ": " + results[0]['count (*)'] );	
		stat = results[0]['count (*)'];
		console.log(stat);
				if (whichStats == 1 || whichStats == 2)
				{
					returnstat = results[0]['count (*)'];
					console.log("returnstat in callback= "+ returnstat);
				}
	...
	console.log("returnstat = "+ returnstat);
	//
	return returnstat;
}
