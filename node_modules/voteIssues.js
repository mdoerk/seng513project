...
function printPage(response) 
{
	...
	{	
		...
		{				
			var sqlInsertNewUser = "INSERT INTO 'votes' ('user_id', 'issue_id', 'vote') "
			...
			dbAccess.runQuery(sqlInsertNewUser, function(error, rows)
			{
				...
				else  //Successfull Vote; Simply go back to the previous page for now
				{
					response.write('<html><script language=\'JavaScript\'>location.replace("/viewIssue?id='+g_IssueId+'")</script></html>');
		...
		{				
			...
			response.write('<html><script language=\'JavaScript\'>alert(\'Sorry you have already voted!\'),location.replace("/viewIssue?id='+g_IssueId+'")</script></html>');
			response.end();		
		}
	}
}
...
VoteIssues.display = function(request, response) 
{
	...
	*/
	 ...
	 g_userId = parsedURL.query.userID;
	...
	if (g_userId == null)
		{
			...
			response.write('<meta http-equiv="REFRESH" content="0;url=./vote_msg_1.html"></HEAD><BODY></BODY></HTML>');
			response.end();		
		}
		else
		{
