...
function displayPage(response, issue, id) {
	return function(error, users)
	{
		...
		{	
			console.log("asdfkjahsdfkjahsdfkajheAKSJDHFALSKDJFHALSDKFJHASDF");
			...
			if (user != undefined){
				response.write('<div id="header">Welcome, <a href="editProfile?id='+user.id+'">'+user.name+'</a> | <a href="signout">Logout</a></div>');
			}
			else {
...
/**
 * Check that the like/dislike counts are not null
 *	if they are, update them and re-read the issue from the database
 * @param response Response object to send the data
 * @param issue Issue to display
 * @param id Issue's Id
 */
function checkVotesBeforeDisplay(response, issue, id)
{	
	if (issue['likes'] == null || issue['dislikes'] == null) {
		console.log("DISPLAY PAGE - needs to updates!!!!!!");
		console.log("Issue votes not up to date, running update for likes/dislikes.");
		voteIssues.updateIssueVotes(issue['id'], function() {
			var getUpdatedIssue = 'SELECT * FROM issues WHERE id='+id+';';
			dbAccess.runQuery(getUpdatedIssue, function(error, updatedissue) {
				if (error) throw error;
				console.log("DISPLAY PAGE - UPDATED ISSUE");
				console.log(updatedissue[0]);
				return displayPage(response, updatedissue[0], id);
			});
		});
	}
	else {
		console.log("DISPLAY PAGE - all is good");
		console.log(issue);
		return displayPage(response, issue, id);
	}
}
...
function findUser(response,id) {
		...
		dbAccess.runQuery('SELECT * FROM users WHERE id=' + issue[0]['user_id'], checkVotesBeforeDisplay(response, issue[0], id));
