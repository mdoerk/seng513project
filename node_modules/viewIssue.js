				...
				dbAccess.runQuery(sqlQuery2, function (error, rows2) {
					...
					getStats(thisIssueId, function(issueStats) {
						getFollow(g_UserId, thisIssueId, function(follows) {
							...
							if (variables.signedIn == null) {
								referral.set(lastSite);
								referral.history(1);
							}
	...
	if (g_UserId == -1) {
		...
		req.on('end', function() {
			...
			if(decodedBody.comments.trim() != ""){
				var sqlQueryInsert = "INSERT INTO comments (id,user_id,issue_id,content) VALUES ("
				+ null + ",'"
				+ g_UserId + "','"
				+ thisIssueId + "','"
				+ sanitizeInputs.removeTags(decodedBody.comments)
				+ "')";
				dbAccess.runQuery(sqlQueryInsert, function (error, rows) {
					if(error) throw error;
					res.statusCode = 200;
					...
					dbAccess.runQuery(sqlQuery, function (error, rows) {
						...
						dbAccess.runQuery(sqlQuery2, function (error, rows2) {
							...
							if (rows.length > 0) {
								...
								for (var i = 0; i <= rows.length - 1; i++) {
									...
									if(rows[i].user_id == g_UserId)
									{
										...
										obj = {
											...
											iCommentCreatorName : name,
											iCommentCreatedTime : dateFormat(rows[i].created, "dddd, mmmm dS, yyyy, h:MM:ss TT"),
											iCommentContent : decodedBody.comments,
											iButtonType : buttonType,
											iCommentId : rows[i].id,
											iIssueId : thisIssueId
										};
								...
								res.render('views/viewComment.html', obj);
							}
						});
					});
				});		
								...
								dbAccess.find('users', { conditions:['id="' + issue_owner[0].user_id + '"']}, function(error, owner){
									...
									if (owner) {
										...
										var body = '<html><body><h4>Issue: ' + issue_owner[0].title + '</h4>' +
												   '<p>' + this_user[0].name + ' comments, "' + 
												   decodedBody.comments + '"</p><br>' + 
