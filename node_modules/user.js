...
exports.getUserFromRequest = function(req, callback){
	...
	var query = "SELECT * FROM users JOIN sessions ON sessions.user_id=users.id WHERE sessions.session_hash='"+session+"';";
	
	dbAccess.runQuery(query, function(error, records){
	//dbAccess.find('sessions', { conditions:['session_hash="' + session + '"']}, function(error, records){
		if(error)
			return callback(error, null);
