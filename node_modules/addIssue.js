...
AddIssue.display = function(req, res) {
	...
	if (req.method.toUpperCase() != 'POST') {
		req.getUser(function(error, user) {
			...
			if (variables.loggedIn == null) {
				referral.set(req.url);
				referral.history(1);
			}
					...
					if (error) { // if an error occurred when inserting a new issue, print out the error
						...
						reputation.updateOnAddIssue(issueId, user.id,
							function(error, results) {
								dbAccess.find('issues', {conditions: ['id="' + issueId +'"' ] }, 
								function(error, results) {
									util.log("results[0].id=" + results[0].id);
									util.log("results[0].user_id=" + results[0].user_id);  
									if (error) {                              
										util.log("error on find issue ");
									} 
									else {  
										res.redirectTo('/listIssues');
									}   
								});
						});  
