/* tags.js 
 *
 * Provides functions for tagging issues
 * Dori Saba
 * sabad66@hotmail.com
 * March 3, 2011
 *
 * Future: Right now, it can only handle one-word tags. So 'tag1 tag2' will be two tags: ['tag1', 'tag2']
 * Eventually, would like to have it so that you can have more than one word tags ie 'tag1 "le tag" tag3'
 * would be ['tag1', 'le tag', 'tag2']. Currently, it would parse to: ['tag1', '"le', 'tag"', 'tag3']
 */
 
var dbAccess = require('dbAccess'),
	sqlite = require('sqlite'),
	util = require('util');

var MAXTAGS = 20; // Set a maximum of 20 tags per issue

/* tagIssue
 * Gets the reputation score of a specified user 
 *
 * 'issueId' The id of the issue
 * 'tags' The tags to tag the issue with
 * 'call_back' The call back function which returns the reputation score of the user 
 */
var tagIssue = exports.tagIssue = function(issueId, tags, call_back) {
	// First we need to check if we have a valid issueId
	dbAccess.runQuery("SELECT * FROM issues WHERE id = " + issueId  + ";", function (error, result) {
		if (error)
			util.log("error in searching for issue to tag. was the issue created successfully?");
		else {
			// Now check that we have one result
			if (result.length != 1) {
				util.log("error... we have more than 1 issue matchign id " + issueId);
				if (call_back instanceof Function) call_back(error);
			}
			else
			{
				// Split it using a regex that checks for one or more spaces
				var tagList = tags.split(/\s+/, MAXTAGS);
				// After much googling, found this nice javascript iteration pattern here:
				// http://metaduck.com/post/2675027550/asynchronous-iteration-patterns-in-node-js
				(function insertOne() {
					var t = tagList.splice(0, 1)[0]; // get the first one and reduce the list by 1
					if (!isNullOrEmptyOrBlank(t)) { // Only proceed if it is a 'good' tag
						addTag(t, function(e2, tagId) { // Add the tag to the db.. the ID of the tag will be returned
							if (e2) {
								util.log("error in tagIssue.. came from addTag" + e2);
								call_back(e2);
							}
							else {
								// Now we create the relationship in the issuetags table
								dbAccess.create('issuetags', { values:['issue_id=\"' + issueId + "\"", 'tag_id=\"' + tagId + "\""] }, function(e3) {
									if (e3) {
										util.log("error creating tag relationship (tags.tagIssue)" + e3);
										call_back(e3);
									}
									else {
										//util.log("relationship from issue " + issueId + " to tag " + tagId + " added successfully");
										if (tagList.length == 0) {
											call_back();
										}
										else {
											process.nextTick(insertOne, 0); // Recursively call this wrapped function
										}
									}
								});
							}
						});
					}
				})();
			}
		}
	});
}

/* getTagId
 * Gets the id of a given tag from the tags table. Returns -1 if it doesn't exit
 * 
 * 'tag' is the id we are checking
 */
var getTagId = exports.getTagId = function(tag, call_back) {
	// Run the select query
	var q = "SELECT id FROM tags where tag = '" + tag + "';"
	dbAccess.runQuery(q, function (error, results) {
		if (error) {
			util.log("error in tags.getTagId1. " + error);
			if (call_back instanceof Function) call_back(error, -1);
		}
		else if (results.length != 0) { // If rows are returned, return the id to the callback
			if (call_back instanceof Function) call_back(undefined, results[0].id);
		}
		else
			if (call_back instanceof Function) call_back(undefined, -1); // otherwise return -1
	});
}

/* addTag
 * Adds the tag to the tags table. Returns the tag id, or -1 if there is an error.
 * 
 * 'tag' is the tag we are adding
 */
var addTag = exports.addTag = function(tag, callback) {
	// Check if it exists
	getTagId(tag, function(error, tagId) {
		if (error) {
			util.log("error in addTag.. came from calling getTagId " + error);
			callback(error, -1);
		}
		else if (tagId == -1) { // If it doesn't exist, insert it into the db
			dbAccess.create('tags', { values:['tag=\"' + tag + "\""] }, function(error, newTagId) {
				if (error) {
					util.log("error in addtag" + error);
					if (callback instanceof Function) callback(error, -1);
				}
				else {
					//util.log("Tag '" + tag + "' added to tags table with id " + newTagId);
					if (callback instanceof Function) callback(undefined, newTagId);
				}
			});
		}
		else {
			//util.log("Tag '" + tag + "' exists with id " + tagId);
			if (callback instanceof Function) callback(undefined, tagId);
		}
	});
}

// Simple function that returns true if the tag is useless (i.e. bunch of whitespace),
// or false if the tag is good
function isNullOrEmptyOrBlank(str) { 
	return (!str || /^\s*$/.test(str));
}

// Looks up the associated tags for a given issueId. Returns the raw database results data to the callback
// Usually you won't need to use this directly... instead use getTagsList or getTagsString
var getTags = exports.getTags = function (issueId, callback) {
	// Get a list of the tags from the db
	var q = "SELECT tag FROM tags WHERE id IN (SELECT tag_id FROM issuetags WHERE issue_id = " + issueId + ") ORDER BY tag;"
	dbAccess.runQuery(q, function (error, results) {
		if (error) {
			util.log("error running query in tags.getTags. " + error);
			if (callback instanceof Function) callback(error);
		}
		else
			if (callback instanceof Function) callback(undefined, results);
	});
}

// Gets the tags associated with an issue and returns a list of strings to the callback
var getTagsList = exports.getTagsList = function(issueId, callback) {
	var tagList = new Array();
	getTags(issueId, function(error, results) {
		if (error) {
			util.log("Error in tags.getTagsList. " + error);
		}
		else {
			for (i = 0; i < results.length; ++i) {
				tagList.push(results[i].tag);
			}
		}
		if (callback instanceof Function) callback(tagList);
	});
}

// Gets the tags associated with an issue and returns a space separated list of the tags to the callback
var getTagsString = exports.getTagsString = function(issueId, callback) {
	var s = '';
	getTags(issueId, function(error, results) {
		if (error) {
			util.log("Error in tags.getTagsString. " + error);
		}
		else {
			if (results.length > 0) {
				s += results[0].tag;
				for (i = 1; i < results.length; ++i) {
					s += ' ' + results[i].tag;
				}
			}
		}
		if (callback instanceof Function) callback(s);
	});
}

// Takes in a tag to search for, and returns a list of ids that are tagged with this tag
var getIssuesByTag = exports.getIssuesByTag = function(tag, callback) {
	var issues = new Array();
	var q = "SELECT issue_id FROM issuetags WHERE tag_id = (SELECT id FROM tags WHERE tag='" + tag + "');";
	dbAccess.runQuery(q, function (error, results) {
		if (error) {
			util.log("Error running select query in tags.getIssuesByTag. " + error);
		}
		else {
			for (i = 0; i < results.length; i++) {
				issues.push(results[i].issue_id);
			}
		}
		if (callback instanceof Function) callback(issues);
	});
}

// Untags a given issue. (i.e. removes all issuetags relationships for the given issueId)
var untagIssue = exports.untagIssue = function(issueId, callback) {
	// This is as simple as deleting all rows in the issuetags table where issue_id = issueId
	dbAccess.remove('issuetags', { conditions:['issue_id="' + issueId + '"'] }, function (error) {
		if (error) {
			util.log('Error in tags.untagIssue. ' + error);
		}
		if (callback instanceof Function) callback(error);
	});
}

/* updateTags
 * Updates the tags of the given issueId with the given new tags
 *
 * 'issueId' The id of the issue
 * 'tags' The updated list of tags
 * 'callback' The callback function which returns the reputation score of the user 
 */
var updateTags = exports.updateTags = function(issueId, newTags, callback) {
	// Not the most efficient way to do it, but it'll do for now
	// Basically just going to remove all tags associated with the issue, then re-tag the issue
	untagIssue(issueId, function(error) {
		if (error) {
			util.log("error in tags.updateTags. " + error);
			if (callback instanceof Function) callback(error);
		}
		else {
			tagIssue(issueId, newTags, function(error) {
				if (error) {
					util.log("error in tags.updateTags2. " + error);
					if (callback instanceof Function) callback(error);
				}
				else {
					callback();
				}
			});
		}
	});
}
