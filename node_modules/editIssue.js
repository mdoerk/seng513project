 ...
 */
function serve(response,id,canEdit) {
    return function(error, rows) {
	  ...
	  response.end('<html><body><h1>404 Not Found</h1></body></html>');
		...
		response.write('<title>CivicConnect</title><link rel="stylesheet" href="css/style.css" type="text/css" /></head>');
			response.write('<body> <div id="page"><div id="header"><a href="signup.html">join</a> | <a href="signin">login</a> </div> <div id="menu"><ul><li><a href="index.html">Home</a></li><li><a href="about.html">About</a></li></ul></div><div id="content">');
		...
		if ( canEdit == 'true' ){
			...
			response.write('<p>Status</p> <SELECT name="status">');
			...
			response.write('You must be logged in as the original poster or an editor to edit this issue. Click <a href="viewIssue?id='+id+'">here</a> to view the issue again.');
...
EditIssue.display = function(request, response) {
	...
	request.getUser(function(error, user){
		...
		if(user){ 
			dbAccess.runQuery(sqlQuery, serve(response,parsedURL.query.id, user.isEditor));
		}
		else		
			dbAccess.runQuery(sqlQuery, serve(response,parsedURL.query.id, 'false'));	
	})
