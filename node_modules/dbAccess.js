...
var sqlite = require('sqlite'),
	...
	DATABASE_NAME = settings.getSetting('db_file');
...
exports.create = function(table, params, call_back)
{
	...
	if (params.values.length>0) {
		...
		for (var index in params.values) {
			...
			values += "?";
			valuesArray[index] = splitResults[1];
		...
		db.open(DATABASE_NAME, function(open_error) {
			...
			db.execute(query, valuesArray, function (execute_error) {
...
exports.find = function(table, params, call_back)
{
	...
	// 1. SELECT WHAT
	var query = "SELECT ";
	var conditionsValuesList = new Array();
	...
	if (params.conditions.length > 0) {
		...
		for (var index in params.conditions) {
			var splitResults = params.conditions[index].split("=");
			query += splitResults[0] + "=?";
			conditionsValuesList[index] = splitResults[1];
			
			if(conditionsValuesList[index].charAt(0) == '\"') {
				conditionsValuesList[index] = conditionsValuesList[index].substr(1);
			}
			if(conditionsValuesList[index].charAt(conditionsValuesList[index].length-1) == '\"') {
				conditionsValuesList[index] = conditionsValuesList[index].substr(0, conditionsValuesList[index].length-1);
			}
	...
	db.open(DATABASE_NAME, function(open_error) {
		...
		console.log("\n\n\nquery: " + query);
		console.log("conditionsValuesList: " + conditionsValuesList + "\n\n\n");
		
		// Execute query
		db.execute(query, conditionsValuesList, function (execute_error, rows) {
			db.close(function () {
...
exports.update = function(table, params, call_back)
{
	...
	if (params.values.length > 0) {
		...
		for (var index in params.values) {
			...
			valuesList[index] = splitResults[1];
			
			if(valuesList[index].charAt(0) == '\"') {
				valuesList[index] = valuesList[index].substr(1);
			}
			if(valuesList[index].charAt(valuesList[index].length-1) == '\"') {
				valuesList[index] = valuesList[index].substr(0, valuesList[index].length-1);
			}
	...
	db.open(DATABASE_NAME, function(open_error) {
		...
		if (open_error != undefined)
			return call_back(open_error);
		// Execute query
		db.execute(query, valuesList, function (execute_error) {
			db.close(function () {
	...
	if (params.conditions.length > 0) {
		...
		db.open(DATABASE_NAME, function(open_error) {
			...
			if (open_error != undefined)
				return call_back(open_error);
			// Execute query
			db.execute(query, conditionsValuesList, function (execute_error) {
				db.close(function () {
					if (call_back instanceof Function) { call_back(execute_error); } 
				});
			});
