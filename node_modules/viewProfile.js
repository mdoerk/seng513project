...
ViewProfile.display = function(request, response)
{
	...
	dbAccess.find('users', { conditions:['id="' + id + '"'] }, function(error, results) 
	{
		...
		else
		{
			...
			response.write('<p><h2>');
			response.write('<strong>E-mail</strong> ' + results[0].email + '<br>');
			response.write('<strong>Neighborhood</strong> ' + results[0].neighborhood + '<br>');
			response.write('<strong>Postal Code</strong> ' + results[0].postal_code + '<br>');
			response.write('</h2>');
		}
