...
ListIssues.display = function(request, response) {
	...
	var sqlQuery = 'SELECT count(*) FROM issues ' + searchQuery(searchParams.search);
...
function getPageCount(searchParams, response) {
	return function(error, rows) {
		...
		var sqlQuery = 'SELECT issues.promotes, issues.status, issues.id, issues.title, issues.lastModified, users.name, issues.user_id, count(comments.id) as total FROM issues LEFT OUTER JOIN comments ON issues.id==comments.issue_id JOIN users ON issues.user_id==users.id ' +
			searchQuery(searchParams.search) +
			' GROUP BY issues.promotes, issues.status, issues.id, issues.title, issues.lastModified ' +
			orderQuery(searchParams.sortBy, searchParams.direction) +
			' limit ' + issuesPerPage + ' offset ' + (searchParams.page - 1) * issuesPerPage;
...
function printPage(searchParams, response) {
    return function(error, rows) {	
			...
			for (i = 0; i < rows.length; i++) {			
				var promote = (rows[i]['promotes'] != null) ? rows[i]['promotes'] : 0;
				var obj = {
					iStatus: rows[i]['status'],
					iId: rows[i]['id'],
					iTitle: rows[i]['title'],
					iLastModified: rows[i]['lastModified'],
					comments: rows[i]['total'],
					authorname: rows[i]['name'],
					authorid: rows[i]['user_id'],
					promotes: promote
				};
				content.push(obj);
        }
