...
EditProfileModule.buildEditProfilePage = function(request, response) 
{
	...
	request.getUser(function(error, user){	
		if (error){
			...
			dbAccess.find('interests', {conditions:['user_id="'+user.id+'"']}, function(error, interests){
				...
				values.textarealimit = 5000;
				values.inputlimit = 150;
...
//		var oldusername = updateInfo['oldusername'];
		var userid = sanitizeInputs.removeTags(updateInfo['userid']);
		var name = sanitizeInputs.removeTags(updateInfo['name']);
		var email = sanitizeInputs.removeTags(updateInfo['email']);
		var neighborhood = sanitizeInputs.removeTags(updateInfo['neighborhood']);
		var postalcode = sanitizeInputs.removeTags(updateInfo['postal_code']);
		var facebookaccount = sanitizeInputs.removeTags(updateInfo['facebook']);
		var twitteraccount = sanitizeInputs.removeTags(updateInfo['twitter']);
		var website = sanitizeInputs.removeTags(updateInfo['website']);
