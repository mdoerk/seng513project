...
var deleteMessage = exports.deleteMessage = function(msgId, folderId, callback) {
	...
	case '1': dbAccess.remove('sent_msgs', { conditions:['id="'+msgId+'"'] }, function(error){
			...
			break;
	default:
		console.log("unknown folderId error in deleteMessage: "+folderId);
...
var getMessages = exports.getMessages = function(userId, folderId, callback) {
	...
	case 1: dbAccess.find('sent_msgs', { conditions:['from_id="'+userId+'"'], orderby:'sent_date desc'},
			...
			break;
	default:
		console.log("unknown folderId error in getMessages: "+folderId);
...
util.log("folder" + folderId);
	...
	case '1': dbAccess.find('sent_msgs', { conditions:['id="'+msgId+'"'], orderby:'sent_date desc'},
			...
			break;
	default:
		console.log("unknown folderId error in getMessage: "+folderId);
		if (callback instanceof Function) callback('Error in messages.getMessage. Invalid folder id passed (' + folderId + ').');
	}
}

var setRead = exports.setRead = function(msgId, folderId, callback) {
	// TODO: set a message as READ (isRead = true)
	switch(folderId) {
	case '0': dbAccess.update('inbox_msgs', { values:['isRead="true"'], conditions:['id="'+msgId+'"']},
							function(error) {
								if(error)
								{
									console.log("error updating message in inbox_msgs");
									if (callback instanceof Function) callback(error);
									else console.log("callback is not a valid function");
								}
									...
									console.log("callback is not a valid function");								
							});
			break;
	case '1': dbAccess.update('sent_msgs', { values:['isRead="true"'], conditions:['id="'+msgId+'"']},
							function(error) {
								if(error)
								{
									console.log("error updating message in sent_msgs");
									if (callback instanceof Function) callback(error);
									else console.log("callback is not a valid function");
								}
								else if (callback instanceof Function) 
			...
			break;
	default:
		console.log("unknown folderId error in setRead");
		if (callback instanceof Function) callback('Error in messages.setRead. Invalid folder id passed (' + folderId + ').');
	}
}
