...
SignupModule.handleSignup = function(req, res) 
{
	...
	req.on('end', function(){
		...
		var sqlInsertNewUser = "INSERT INTO 'users' ('name', 'email', 'password', 'isEditor') "
				+ "VALUES ('" + username + "', '" + email + "', '" + cryptoPassword + "', '" + 'true' + "');";
		dbAccess.runQuery(sqlInsertNewUser, function(error, rows){
			if(error) {
				...
				//find id of newly created user
				var sqlGetNewUserId = "SELECT id FROM users WHERE name='"+username+"';";
				dbAccess.runQuery(sqlGetNewUserId, function(error, results){
//					console.log("User '" + username + "' was succesfully created, id = " + results['id']); 
//					res.setHeader('Set-Cookie', 'session=' + results['id']);
