...
EditProfileModule.buildEditProfilePage = function(request, response) 
{
	...
	dbAccess.find('users', { conditions:['id="' + userid + '"'] }, function(error, results){
		if (error){
			...
			response.write('<h1>EDIT YOUR PROFILE HERE!</h1>');
...
//		var userid = updateInfo[...];
		...
		// Verify old password
		var sqlUpdateUserProfile = "SELECT * FROM users WHERE name='" + oldusername +"';";
		dbAccess.runQuery(sqlUpdateUserProfile, function(error, rows){
			if(error) {
				...
				if (oldCryptoPassword != dbpassword){
					...
					res.end('<html><body><h1>Incorrect Password</h1></body></html>');
				}
				else {
					//updateUserProfile();
