...
EditProfileModule.buildEditProfilePage = function(request, response) 
{
	...
	request.getUser(function(error, user){	
		if (error){
			...
			userid = user.id;
			var username = user.name;//results[0].name;
			var email = user.email;//results[0].email;
			var neighborhood = user.neighborhood; //results[0].neighborhood;
			var postal_code = user.postal_code;//results[0].postal_code;
			...
			displayUserProfileForm(response, userid, username, email, neighborhood, postal_code);
			
			//get Interest for user
			dbAccess.find('interests', {conditions:['user_id="'+userid+'"']}, displayUserInterestForm(response,userid));
...
function displayHeader(response){
			...
			response.write('<li><a href="editProfile">My Profile</a></li>');
            response.write('<li><a href="about.html">About</a></li>');
            response.write('</ul>');
            response.write('</div>');
            response.write('<div id="quick-task-bar">');
		        response.write('<a href="/listIssues" id="add-issue-button">List Issue</a>');
...
*/
function displayFooter(response){
// End response
			response.write('</div> <div id="footer"><p>&copy; 2011 University of Calgary. All rights reserved.</p></div></div></body> </html>');
			response.end();
}

function displayUserProfileForm(response, userid, username, email, neighborhood,postal_code)
{
// Build page content, with user values pre-populated.							
		response.write('<table width = "100%" align ="top">'); 
			response.write('<tr valign=top>');
			response.write('<td width="50%" align = "top">');
			response.write('<table width = "100%" align ="top">');
			response.write('<tr>');
			response.write('<td><h1>Edit Your Profile</h1></td></tr>');
			response.write('<tr>');
			response.write('<td>');
			...
			response.write('<p>Your Facebook Account<br> <input type="text" name="facebook" value="" /></p>');
			response.write('<p>Your Twitter Account<br> <input type="text" name="twitter" value="" /></p>');
			response.write('<br>');
...
/*
displayUserInterestForm
display the form for user to enter interest data
*/
function displayUserInterestForm(response, user_id){
	return function(error,interests){
		if(error) throw error;
		response.write('<td width ="50%" align="top">');
		response.write('<table width = "100%" align ="top">');
			response.write('<tr>');
			response.write('<td align="top"><h1>Edit Your Interest</h1></td></tr>');
			response.write('<tr>');
			response.write('<td align="top">');
		if(interests.length> 0){
			var interest = interests[0];
			
			//interest found fill in the form  
			      response.write('<form id="editInterest_form" action="saveInterest" method="post" enctype="application/x-www-form-urlencoded" >');
			      response.write('<p>Topics:<br>(list multiple topic interest seperated by comma)<br><textarea rows="6" cols="40" type="text" name="interest_topic" >' + interest['interest_topic'] + '</textarea></p>');
			      response.write('<p>Location:<br>(list multiple location interest seperated by comma)<Br><textarea rows="6" cols="40" type="text" name="interest_location">' + interest['interest_location'] + '</textarea></p>'); 
			      response.write('<input type=HIDDEN name="user_id" value="'+user_id +'">');
            response.write('<br>'); 
			      response.write('<input type="submit" value="Update Interest" />');
			      response.write('</form>');
			
		}else{
		 //no interest found, empty form 
			      response.write('<form id="editInterest_form" action="saveInterest" method="post" enctype="application/x-www-form-urlencoded" >');
			      response.write('<p>Topics:<br><textarea rows="13" cols="80" type="text" name="interest_topic" > </textarea></p>');
			      response.write('<p>Location:<br><textarea rows="13" cols="80" type="text" name="interest_location"> </textarea></p>'); 
			      response.write('<input type=HIDDEN name="user_id" value="'+user_id +'">');
				response.write('<br>'); 
			      response.write('<input type="submit" value="Update Interest" />');
			      response.write('</form>');  
