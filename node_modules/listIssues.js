...
function getNumPageTotal(numPage, sortBy, response) {
    return function(error, rows) {
        ...
        var sqlQuery = 'SELECT * FROM issues' + sortBy + ' limit ' + numIssuePage + ' offset ' + (numPage - numIssuePage) * numIssuePage;
        dbAccess.runQuery(sqlQuery, printPage(numPage, numPageTotal, response));
    };
}
 ...
 * @param response Response object to send data to the client
 */
function printPage(numPage, numPageTotal, response) {
    return function(error, rows) {	
        if (error) throw error;
	    ...
	    response.write('<head>');
            ...
            response.write('<div id="quick-task-bar">');
		        response.write('<a href="/add.html" id="add-issue-button">Add Issue</a>');
            ...
            response.write('<a href="/?page=1">First</a> ');

            /* Previous page */
            if (numPage - 1 > 0) {
                response.write('<a href="/?page=' + (numPage - 1) + '"><</a> ');
            }
            ...
            if (parseInt(numPage) + 1 <= numPageTotal) {
                response.write('<a href="/?page=' + (parseInt(numPage) + 1) + '">></a> ');
            }

            response.write('<a href="/?page=' + numPageTotal + '"> Last</a>');
            response.write('</div>');
...
ListIssues.display = function(request, response) {
    ...
    var sortBy = sortQuery(parsedURL.query['sortby'])
