    ...
    req.on('end', function() {
        ...
        dbAccess.find('users', { conditions:['name="' + username + '"']}, function(error, records){
            ...
            if (records.length != 0) {    // username already exists
                console.log('Sorry, username already exists. Please enter another');
                res.render('views/signup.html', {error: 'Signup Failed - username: ' + username + ' already exists.'});
                    ...
                    if(error) {
                        ...
                        dbAccess.find('users', { conditions:['name="' + username + '"']}, function(error, records){
                            ...
                            if (records) {
								...
								var sessionString = generateHash(records[0].id);
