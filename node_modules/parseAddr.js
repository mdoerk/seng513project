...
var geocode = exports.geocode = function(address, call_back) {
	...
	request.on('response', function (response) {
		...
		response.on('end', function() {
			/* Google Maps returns a JSON object with no name which 
			 * is determined as "undefined" in JavaScript
			 * Therefore here is treating the object as a string and 
			 * removing the first 9 chars "undefined" then rebuild
			 * the string as an object
			 */
			res = new String(res.substr(9));
			res = eval('(' + res + ')');
			switch(res.status) {
				case "OK": 
